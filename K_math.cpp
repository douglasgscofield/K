...
static KScalar a_lnbinomial[binomial_N_VALS + 1][binomial_K_VALS + 1] =
	{ -1.0 };
...
*/
{
	...
	if (k < 0 || n < 0 || k > n) {
		char buf[200];
		sprintf(buf, "%s: args incorrect: n=%d, k=%d", thisfunction, n, k);
		fatal(buf);
	}
	...
	}
	{
		...
		for (nn = 0; nn <= binomial_N_VALS; nn++) {
			...
			for (kk = 2; kk <= max_kk; kk++) {
				a_lnbinomial[nn][kk] =
					lnfactorial(nn) - lnfactorial(kk) - lnfactorial(nn - kk);
			}
		}
		done_lnbinomial = 1;
	}
