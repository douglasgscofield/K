...
/* ///////////////////////////////////////////////////////////// */
void
set_repro_resources_kondrashov(
	KConfig K)
/*
** sets up the arrays that drive the reproductive resource algorithms
*/
				...
				for (n = i; n <= K->MI; n++) {
					for (v = 0; v <= j; v++) {
						...
						IF_DEBUG(DEBUG_TRACE3)
							printf("%s: [%d][%d] <- [%d][%d]\n", thisfunction,
								   i, j, n, v);
						sum += s_self(i, j, n, v) * K->S[g] * loadclass;
					}
				}
...
/* ///////////////////////////////////////////////////////////// */
void
compute_self_progeny_kondrashov(
	KConfig K)
/*
** Compute proportions of selfed genotypes produced by population.
				...
				for (n = 0; n <= K->MI; n++) {
					for (v = 0; v <= K->MJ; v++) {
						...
						if (i <= n && j >= v && j <= n + v && i + j <= n + v) {
							t1 = K->gamma[n][v] * s_self(i, j, n, v);
							innersum = 0.0;
							for (xi = 0; xi < K->genotypes; xi++) {
								innersum += K->SO[n][v][xi] * K->trfm_S[xi][g];
							}
							sum += t1 * innersum;
						}
					}
	...
	for (g = 0; g < K->genotypes; g++) {
		...
		for (i = 0; i <= K->MI; i++) {
			for (j = 0; j <= K->MJ; j++) {
				...
				to[i][j][g] =
					a_apomixis(i, j, n, v) * K->A[xi] * from[n][v][xi];
			}
		}
	}
}
...
/* ///////////////////////////////////////////////////////////// */
void
compute_apomixis_progeny_kondrashov(
	KConfig K)
/*
** Compute proportions of selfed genotypes produced by population.
** This is related to Kondrashov 1985, p.640, eq. (2), x"i(k) part,
...
/* ///////////////////////////////////////////////////////////// */
void
compute_outcross_progeny_kondrashov(
	KConfig K)
/*
...
*/
{
	...
	for (i = 0; i <= K->MI; i++) {
		sum = 0.0;
		for (k = 0; k <= i; k++) {
			sum += ((mgam[k] * fgam[i - k]) + (mgam[i - k] * fgam[k]));
		}
...
/* ///////////////////////////////////////////////////////////// */
void
compute_auxiliary_values_kondrashov(
	KConfig K)
{
	...
	for (i = 0; i <= K->MI; i++) {
		for (j = 0; j <= K->MJ; j++) {
			...
			for (g = 0; g < K->genotypes; g++) {
				if (SOsum != 0.0)
					K->SO[i][j][g] = K->xp[i][j][g] * K->rsrc_SO[g] / SOsum;
				else
					K->SO[i][j][g] = 0.0;
				if (AOsum != 0.0)
					K->AO[i][j][g] = K->xp[i][j][g] * K->rsrc_AO[g] / AOsum;
				else
					K->AO[i][j][g] = 0.0;
				if (OOsum != 0.0)
					K->OO[i][j][g] = K->xp[i][j][g] * K->rsrc_OO[g] / OOsum;
				else
					K->OO[i][j][g] = 0.0;
				if (OPsum != 0.0)
					K->OP[i][j][g] = K->xp[i][j][g] * K->rsrc_OP[g] / OPsum;
				else
					K->OP[i][j][g] = 0.0;
			}
		}
