				...
				for (i0 = 0; i0 <= KN->MI0; i0++) {
					...
					IF_DEBUG(DEBUG_TRACE2)
						if (!(i0 % 10) && !(j0 % 10) && !(i1 % 10) &&
							!(j1 % 10))
						printf("mut[%d,%d,%d,%d] ", i0, j0, i1, j1);
#endif
					sum = 0.0;
					for (n1 = 0; n1 <= i1; n1++) {
						for (n0 = 0; n0 <= i0; n0++) {
	...
	for (g = 0; g < K->genotypes; g++) {
		for (j = 0; j <= K->MJ; j++) {
			for (i = 0; i <= K->MI; i++) {
				sum = 0.0;
				for (n1 = 0; n1 <= i; n1++) {
					sum += from[n1][j][g] * mut_term_general(U, i - n1);
				}
				to[i][j][g] = sum;
			}
		}
...
*/
{
	...
	if (U == prev_U) {
		...
		if (i >= pois_general_MAX) {
			char buf[200];
			sprintf(buf, "%s: pois_general_MAX too low", thisfunction);
			fatal(buf);
		}
		/* create new entry */
		last_index = i;
		...
		IF_DEBUG(DEBUG_NORMALIZATION) {
			...
			for (i = 0; i <= MAX_MI; i++) {
				...
				printf
					("pois_general[last_index][%d] = %lg, cumulative sum = %lg\n",
