	...
	IF_DEBUG(DEBUG_NORMALIZATION) {
		if (!isOK_KArray_n(KN, a)) {
			printf("%s: %s not normalized: %f, tolerance=%lg\n", caller,
				   array_name, sum_KArray_n(KN, a), NORMALIZATION_TOLERANCE);
		}
	}
}
	...
	KInt mj1)
{
	...
	if (mj1 > KN->MJ1 || mj1 < 0)
		mj1 = KN->MJ1;
	printf("%s BEGIN KArray_n @ 0x%08x; max i0=%d, j0=%d, i1=%d, j1=%d\n",
		   thisfunction, a, mi0, mj0, mi1, mj1);
	printf("i0\tj0\ti1\tj1\ta[i0][j0][i1][j1]\n");
	dump_values_KArray_n(KN, stdout, a, mi0, mj0, mi1, mj1);
	printf("%s END for KArray_n a @ 0x%8x\n", thisfunction, a);
}

/* ///////////////////////////////////////////////////////////// */
void
dump_values_KArray_n(
	KConfig_n KN,
	FILE * fp,
	KArray_n & a,
	KInt mi0,
	...
	KArray_n & a)
{
	...
	printf("%s BEGIN KArray_n @ 0x%08x; dump_KArray_n follows\n", thisfunction,
		   a);
	...
	KInt mi1)
{
	...
	if (mi1 > KN->MI1 || mi1 < 0)
		mi1 = KN->MI1;
	printf("%s BEGIN KVector_n @ 0x%08x; max i0=%d, i1=%d\n", thisfunction, v,
		   mi0, mi1);
	printf("i0\ti1\tv[i0][i1]\n");
	for (i0 = 0; i0 <= mi0; i0++) {
		for (i1 = 0; i1 < mi1; i1++) {
			printf("%d\t%d\t%lg\n", i0, i1, v[i0][i1]);
		}
	}
	printf("%s END for KVector_n v @ 0x%8x\n", thisfunction, v);
}
	...
	KVector_n & v)
{
	...
	printf("%s BEGIN KVector_n @ 0x%08x; dump_KVector_n follows\n",
		   thisfunction, v);
