...
#define  TRAJECTORY_END_OF_GEN    1
  ...
  public:
	...
	const std::string & type);
	static const std::string & match_type(
	int type);
};
	...
	KConfig K)
{
	...
	if (type = TRAJECTORY_END_OF_GEN) {
		...
		KArray & a = K->x;
		file << std::
			endl << "Trajectory" << "\tgen:" << this_gen << "\ttype:" << type
			<< "\tfreq:" << freq << "\twritenow:" << writenow_flag <<
			"\tdropzero:" << dropzero_flag << "\tatequilibrium:" <<
			is_equilibrium(K)
			<< "\theterozygotes:" << "0" << "," << K->
			MI << "\thomozygotes:" << "0" << "," << K->MJ << std::endl;
