...
double calcd, calch, calcr, s, U, tmp, tmp1, tmp2, tmp3, n, q, H, D, R, mu, h,
	wbarout, wbarself, delta, rstar, r, Hstar, Dstar, Rstar, Hstar2, Dstar2,
	Rstar2;
long gens;
int done;
...

{
	...
	if (r > 1 || r < 0)
		nrerror("Selfing rate out of bounds");
	if (s > 1 || s < 0)
		nrerror("Selection coefficient out of bounds");
	if (h > 0.5 || h < 0)
		nrerror("Dominance out of bounds");
	...
	while (done != 1) {
		 ...
		 */
		if (gens > 20)
			if (fabs(delta / tmp1 - 1) < CRIT)
				if (fabs(tmp2 / (2 * n * H) - 1) < CRIT)
					if (fabs(tmp3 / (2 * n * R) - 1) < CRIT)
						done = 1;
		if (gens > 100000)
			done = 1;
	}
...
//    fprintf(outfile2, "%ld %lf %g %lf %lf %lf %lf %lf %lf\n", gens, D, 2*H, wbarout,wbarself,delta, rstar, 2*n*H, H);
	...
	printf
		("Gens		%ld\nD		%lf\n2H		%g\nR		%g\nmeanout		%lf\nmeanself	%lf\nmeanfit		%lf\ndelta		%lf  %lf\nrstar		%lf\nmeanhet		%lf  %lf\nmeanhom		%lf  %lf\nq		%g\nCharl. U	%lf    %lf\n",
		 ...
		 H + R,
		 -2 * log(1 -
				  delta) / (1 - 2 * h), 4 * h * log(1 - delta) / (2 * h - 1));
	...
	)
{
	...
	tmp =
		(-h * s + h * s * mu * mu + 3 * s / 2 - s * mu -
		 0.5 * s * mu * mu) * H - s - 2 * D * mu * h * s +
		2 * D * mu * mu * h * s + 1 + s * D - s * D * mu * mu;
