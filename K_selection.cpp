...
/*///////////////////////////////////////////////////////////////*/
void        apply_selection    (KConfig K, KArray& to, KArray& from)
/*
...
*/
{
    ...
    KScalar w;
    if (K->fitness_function == FITNESS_INVALID ||
        K->fitness_function < 0 || 
        K->fitness_function > fitness_function_VALS) {
        char buf[200];
        sprintf(buf, "%s: model not set or out of range", 
                thisfunction);
        fatal(buf);
    }
...
*/
{
    ...
    for (i=0; i <= K->MI; i++) {
        for (j=0; j <= K->MJ; j++) {
            for (g=0; g < K->genotypes; g++) {
                K->fitness_precomputed[i][j][g] = 
                    fitness_computed(K, i, j, g);
            }
        }
    }
}
...
*/
{
    ...
    if (MAX_fitness_function > fitness_function_VALS) {
        char buf[200];
        sprintf(buf, "%s: exceeded fitness_function_VALS",
                thisfunction);
        fatal(buf);
    }
    fitness_functions[MAX_fitness_function].thisnum = 
        MAX_fitness_function;
...
/*///////////////////////////////////////////////////////////////*/
KPtr_ff     get_fitness_function    (KInt ff)
/*
