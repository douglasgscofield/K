	...
	KInt MJ1)
{
	...
	if (MI0 > MAX_MI_n) {
		strcat(buf, "MI0 too large, adjust MAX_MI_n; ");
		err++;
	} else if (MI0 < 0) {
		strcat(buf, "MI0 too small, <0; ");
		err++;
	}
	if (MJ0 > MAX_MJ_n) {
		strcat(buf, "MJ0 too large, adjust MAX_MJ_n; ");
		err++;
	} else if (MJ0 < 0) {
		strcat(buf, "MJ0 too small, <0; ");
		err++;
	}
	if (MI0 < MJ0) {
		strcat(buf, "MI0 < MJ0; ");
	...
	if (MI1 > MAX_MI_n) {
		strcat(buf, "MI1 too large, adjust MAX_MI_n; ");
		err++;
	} else if (MI1 < 0) {
		strcat(buf, "MI1 too small, <0; ");
		err++;
	}
	if (MJ1 > MAX_MJ_n) {
		strcat(buf, "MJ1 too large, adjust MAX_MJ_n; ");
		err++;
	} else if (MJ1 < 0) {
		strcat(buf, "MJ1 too small, <0; ");
		err++;
	}
	if (MI1 < MJ1) {
		strcat(buf, "MI1 < MJ1; ");
	...
	if (!KN->option_nolethal) {
		if (KN->fit_s[0] == 1.0) {
			IF_DEBUG(DEBUG_LETHALS)
				printf
				("%s: mutation class 0 is lethal, so KN->is_lethal[0]=1\n",
				 thisfunction);
			KN->is_lethal[0] = 1;
			KN->createlethal[0] = 0;
		}
		if (KN->fit_s[1] == 1.0) {
			IF_DEBUG(DEBUG_LETHALS)
				printf
				("%s: mutation class 1 is lethal, so KN->is_lethal[1]=1\n",
				 thisfunction);
			KN->is_lethal[1] = 1;
			KN->createlethal[1] = 0;
		}
		...
		IF_DEBUG(DEBUG_LETHALS)
			printf
			("%s: if either of these are non-zero, expect normalization problems\n",
			 thisfunction);
	}
}
