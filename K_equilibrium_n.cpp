...
is_equilibrium_n(
	KConfig_n KN)
/* 
** return 1 if KN satisfies equilibrium constraints as defined by use
*/
{
...
*/
{
	...
	if (diff_epsilon_KArray_n(KN, adults, prevgen, KN->epsilon)) {
		IF_DEBUG(DEBUG_TRACE1) printf("%s: no equilibrium\n", thisfunction);
		return 0;
	} else {
		IF_DEBUG(DEBUG_TRACE1) printf("%s: at equilibrium\n", thisfunction);
		return 1;
	}
}
...
*/
{
	...
	IF_DEBUG(DEBUG_EQUILIBRIUM) {
		...
		printf("%s: epsilon = %lg, max diff @ [%d][%d][%d][%d] = %lg\n",
		...
		for (j0 = 0; j0 <= KN->MJ0; j0++) {
			for (i1 = 0; i1 <= KN->MI1; i1++) {
				for (j1 = 0; j1 <= KN->MJ1; j1++) {
					if (fabs(a1[i0][j0][i1][j1] - a2[i0][j0][i1][j1]) >
						epsilon) {
